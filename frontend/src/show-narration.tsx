...
import { getStory } from "../../backend/src/text-gen"; // Use the getStory function from text-gen.ts
...
const LandingPage: React.FC = () => {
  ...
  const handleKilledPlayerSelection = (
  ...
  const handleHealedPlayerSelection = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
  ...
  const generateStory = async () => {
    if (
      ...
      setError(
        "Please ensure all player names are filled, both players are selected, and a setting is specified."
      );
      return;
    }
    ...
    if (killedPlayer === healedPlayer) {
      prompt = `Write a story about a group of people. The setting is ${setting} and there were originally ${numPlayers} players. The players' names are ${playerNames.join(
        ", "
      )}. During the night, in our story, ${killedPlayer} was almost killed by 'the mafia'. Write the story relative to our setting. Make it interesting and funny. Keep it under 50 words. Involve at least 3 characters in the story. Use misdirection as to not reveal who almost died until the end of the story. It should be clear that the medic saved the person and that nobody died. Keep it under 50 words`;
    } else {
      prompt = `Write a story about a murder. The setting of this murder is a ${setting} and there were originally ${numPlayers} players. The players' names are ${playerNames.join(
        ", "
      )}. During the night, in our story, ${killedPlayer} was killed by 'the mafia'. Write the story of the mafia killing this person relative to our setting. Make it interesting and funny. Keep it under 50 words. Involve at least 3 characters in the story. Use misdirection as to not reveal who died until the end of the story. It should be clear who died in the story. Keep it under 50 words`;
    }
  ...
  return (
    <div className="landing-container">
      <header className="landing-header">
        ...
        <p>
          Select the number of players (5-15), enter their names, choose a
          killed and healed player, and specify the setting:
        </p>
        ...
        <div>
          ...
          <select
            value={killedPlayer || ""}
            onChange={handleKilledPlayerSelection}
          >
            <option value="" disabled>
              Select a killed player
            </option>
            {playerNames.map(
              (name, index) =>
                name && (
        ...
        <div>
          ...
          <select
            value={healedPlayer || ""}
            onChange={handleHealedPlayerSelection}
          >
            <option value="" disabled>
              Select a healed player
            </option>
            {playerNames.map(
              (name, index) =>
                name && (
